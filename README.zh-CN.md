...
2. **连接 Runtime** — 打开项目和 **DSH** 工具窗口。本地启动需要 Node.js 24+，以及 `pnpm` 或 `npm`/`npx`；插件默认自动启动 Runtime。启动命令或已有 Runtime 的 **Server URL** 可在 **Settings → Tools → DeepSeek Harness** 中配置。
...
| 设置项 | 默认值 | 说明 |
| --- | --- | --- |
| 命令 / 参数 | `auto` / 空 | 优先使用兼容的本机 `dsh`，开启自动安装时回退到固定版本的 pnpm/npx；也支持显式指定命令。 |
